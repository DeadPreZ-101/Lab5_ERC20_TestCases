...
contract("StandardERC20", (accounts) => {
  ...
  // New spender balance
  const increasingSpenderAmount = web3.utils.toBN(211222333444555666);
  const decreasingSpenderAmount = web3.utils.toBN(199888777666555444);
  ...
  it("test transferFrom()", async () => {
    ...
    console.log(oldReceipient1Balance)
   ...
   it("test increaseAllowance()", async () => {
    const oldAllowanceTx = await standardERC20Instance.allowance(receipient1, spender);
    
    const newAllowanceTx = await standardERC20Instance.increaseAllowance(spender, increasingSpenderAmount, {from:receipient1});
   
    const newAllowanceBalance = await standardERC20Instance.allowance(receipient1, spender);

    console.log('***** Increasing Allowance ****')
    console.log(newAllowanceTx);
    console.log(oldAllowanceTx);
    console.log(newAllowanceBalance);
  // const receipient1Balance = await standardERC20Instance.balanceOf.call(receipient1);
  // console.log(receipient1Balance);  
   });

  it("test decreaseAllowance()", async () => {
    const oldAllowanceTx = await standardERC20Instance.allowance(receipient1, spender);

    const newAllowanceTx = await standardERC20Instance.decreaseAllowance(spender, decreasingSpenderAmount, {from:receipient1});
